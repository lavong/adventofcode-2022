...
fn main() {
    let input = fs::read_to_string("src/bin/day25.txt").unwrap();

    let sum_decimal = input.lines().map(decimal).sum();
    let sum_snafu = snafu(sum_decimal);

    println!("Solution: {sum_snafu}");
}

fn decimal(s: &str) -> usize {
    s.chars().fold(0, |acc, c| {
        acc * 5 + "=-012".chars().position(|cc| cc == c).unwrap() - 2
    })
}
