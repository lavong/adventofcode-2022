...
fn main() {
    ...
    for _ in 0..rounds {
        for i in 0..monkeys.len() {
            ...
            for item in monkey.1.iter() {
                let worry_level = match monkey.2 {
            ...
            inspections[i] += monkeys[i].1.len() as u32;
            monkeys[i].1.clear();
        }
    }
    ...
    println!("Solution: {:?}", inspections[0] * inspections[1]);
}
...
fn parse(input: &String) -> Vec<(u32, Vec<u32>, Operation, u32, (usize, usize), u32)> {
    let chunks = input.lines().chunks(7);
    let monkeys = chunks.into_iter().map(|mut chunk| {
        let name: u32 = chunk.next().unwrap()[6..]
            .trim()
            .split(':')
            .next()
            .unwrap()
